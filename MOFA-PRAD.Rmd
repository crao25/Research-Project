...
```{r meth_downsize}
meth.var <- apply(meth.PRAD$dat, 1, function(x) var(x, na.rm=TRUE))
meth.data.PRAD <- subset(meth.PRAD$dat, meth.var >= quantile(meth.var, 0.90, na.rm = TRUE))
```

```{r rnaseq_downsize}
rnaseq.var <- apply(rnaseq.PRAD$dat, 1, function(x) var(x, na.rm=TRUE))
rnaseq.data.PRAD <- subset(rnaseq.PRAD$dat, rnaseq.var >= quantile(rnaseq.var, 0.90, na.rm=TRUE))
```

```{r mut_downsize}
mut.var <- apply(mut.PRAD$dat, 1, function(x) var(x, na.rm=TRUE))
mut.data.PRAD <- subset(mut.PRAD$dat, mut.var >= quantile(mut.var, 0.90, na.rm=TRUE))
```

```{r cna_downsize}
cna.var <- apply(cna_PRAD$dat, 1, function(x) var(x, na.rm=TRUE))
cna.data.PRAD <- subset(cna_PRAD$dat, cna.var >= quantile(cna.var, 0.90, na.rm=TRUE))
```

### Take delta values for methylation and RNAseq (tumor - normal)

```{r delta_meth}
meth.PRAD.tum.norm <- TumorNormalMatch(meth.data.PRAD)
...
```{r delta_rnaseq}
rnaseq.PRAD.tum.norm <- TumorNormalMatch(rnaseq.data.PRAD)
...
### RemPRADe NA values from Methylation data
...
## string4 <- colnames(cna.PRAD$dat)
## colnames(cna.PRAD$dat) <- substring(string4, 1, 12)
...
```{r}
ModelOptions <- getDefaultModelOptions(PRAD_data)
