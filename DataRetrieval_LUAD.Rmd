...
cna.var <- apply((assays(cna_LUAD)[[2]]), 1, var)
cna.data.LUAD <- subset((assays(cna_LUAD)[[2]]), cna.var >= quantile(cna.var, 0.99, na.rm=T) & !is.na(cna.var))
...
cna_tumors <- replace(cna.data.LUAD, cna.data.LUAD < 0, -1)
...
clin_data.LUAD <- data.matrix(cna.LUAD$clinical)
```
...
DT::datatable(head(assays(cna_LUAD)[[2]]))
```
