...
```{store datatables, echo=FALSE, message=FALSE}
rnaseq.data <- datatable(rnaseq.COAD$dat)
meth.data <- datatable(meth.COAD$dat)
mut.data <- datatable(mut.COAD$dat)
cna.data <- datatable(cna_COAD$dat)
```

##Get matched tumor and normal samples

```{r get matched tumor/normal smaples, echo = FALSE}
rnaseq.COAD.tum.norm <- TumorNormalMatch(rnaseq.data)
meth.COAD.tum.norm <- TumorNormalMatch(meth.data)
...
```{r subtract normal from tumor, echo=FALSE, message=FALSE}
meth_new_data <- meth_tum - meth_norm
rnaseq_new_data <- rnaseq_tum - rnaseq_norm

```

```{r new data storing, echo=FALSE, message=FALSE}
data <- list(rnaseq_new_data, meth_new_data, mut_data, cna_data)
```
