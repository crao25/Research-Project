...
```{r cna_downsize}
cna.var <- apply(cna.COAD$dat, 1, function(x) var(x, na.rm=TRUE))
cna.data.COAD <- subset(cna.COAD$dat, cna.var >= quantile(cna.var, 0.90, na.rm=TRUE))
```
...
```{r store_clinical}
clin_data.COAD <- data.matrix(cna.COAD$clinical)
```
